...
void *ThreadBehavior(void *t_data)
{
    ...
    // cykliczne odbieranie wiadomości i wypisywanie ich na konsolę
    while(1){
        //pthread_mutex_lock(&(*th_data).mutex);
        memset((*th_data).msg, 0, sizeof((*th_data).msg));
        read((*th_data).soc, (*th_data).msg, sizeof((*th_data).msg));
        //printf("\n");
        for (int i=0; i<32; i++){
            printf("%c", (*th_data).msg[i]);
        }
...
void handleConnection(int connection_socket_descriptor) {
    ...
    //blokada wątku - będzie wykorzystana dla bezpieczeństwa odczytu/zapisu
    pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;

    //TODO
    t_data.soc = connection_socket_descriptor;
    t_data.mutex = mutex;
    ...
    // cykliczne wysyłanie na serwer wiadomości z konsoli
    while(1) {
        //pthread_mutex_lock(&mutex);
        memset(t_data.msg, 0, sizeof(t_data.msg));
        fgets(t_data.msg, sizeof(t_data.msg), stdin);
    
        write(t_data.soc, t_data.msg, sizeof(t_data.msg));
